...
The purpose of this document is to present the technical design of a robotic arm that can perform handwriting tasks in a physical environment for basic sentences using fine-tuning via reinforcement learning (RL) on a Vision-Language Action model (VLA).
...
We can write each character given their stroke-by-stroke physical definitions, and collect their datapoints (for each stroke of a given character) through the Trossen Data Collection framework.

\begin{verbatim}
"Write 'A'"
-> parse -> ['A']
-> for each character:
   -> look up stroke count, physical motions in defined constant
   -> for each stroke in range(stroke_count):
      -> start recording an 'episode' for data collection
      -> collect current frame that the robot sees
      -> draw stroke using trossen library
      -> collect the 6-DOF tuple / action that is used to draw the stroke
      -> the data point is the writing instruction, frame, action (stroke)
   -> scripted pen-lift + reposition to next character start position
-> done
\end{verbatim}

\includegraphics[width=0.8\textwidth]{assets/vla_data_collection.png}
...
-> for each character:
   ...
   -> run VLA inference stroke_count times WITH fine-tuned model
   -> scripted pen-lift + reposition to next character start position
-> done
