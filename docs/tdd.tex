...
% This is to be removed potentially on completion of this document.
\subsection*{Software}

\begin{enumerate}

  \item \textbf{VNC Setup} \\
  Set up VNC for those who have not yet connected to the server's desktop environment (a graphical environment to observe what is happening inside the Linux container). Only one person should connect at a time via TigerVNC. Everyone who has not completed this setup should prioritize it; all others should focus on the tasks below.

  \item \textbf{Branch Merge} \\
  Migrate correct code from Anling's branch/script/environment to the main branch. Development can continue on this branch going forward.
  ...
  \begin{itemize}
    \item Data points will be collected through Isaac Lab as recordings of the robot performing action $X$ given camera observation $Y$ and text input $Z$. Once collection is complete (HDF5 format), the dataset must be converted to \texttt{LeRobotDataset} format, as required by NORA 1.5 fine-tuning.
    \item Prior to collection, the Isaac Lab virtual environment must be configured to match the physical environment as closely as possible, including arm attachments (Logitech camera), and environmental objects (whiteboard, marker holder, etc.).
